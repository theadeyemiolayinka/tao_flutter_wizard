...
lib/
  ...
  l10n/
    app_en.arb         ← Merged base ARB (auto-populated by merge script / feature hook)
    l10n.yaml          ← flutter gen-l10n config
    app_localizations.dart ← Output of flutter gen-l10n (committed to VCS)
  scripts/
    merge_l10n.dart    ← Re-merges all feature ARBs → app_en.arb (run before gen-l10n)
  main.dart            ← (optional) generated by core brick with --setup_main true
  app.dart             ← (optional) generated App widget wired to AppBloc + GoRouter
  ...
  features/{feature}/
    ...
    l10n/              ← Per-feature ARB files (source of truth for feature strings).
```
...
Ensure your `.gitignore` excludes the appropriate Mason and Flutter directories:
```gitignore
# Mason
.mason/

# Flutter/Dart standard
.dart_tool/
build/
.pub-cache/
.pub/
...
# Optional: Code Generation outputs
# Un-comment below if you prefer NOT committing code gen files
# *.g.dart
# *.freezed.dart
```
...
**`setup_main` flag:** when `true` the hook generates two additional files:

| File | Description |
|---|---|
| `lib/main.dart` | Full entry point: `HydratedBloc` storage init, `AppBlocObserver`, `registerCoreDependencies`, `// mason:core-feature-regis` anchor for feature DI calls, `runApp(const App())` |
| `lib/app.dart` | `App` widget — `BlocProvider<AppBloc>` + `BlocBuilder` driving `MaterialApp.router` with `AppTheme` and `appRouter`. Falls back to a plain `MaterialApp` when `app_bloc` is `false` |

> Feature bricks auto-patch `lib/main.dart` by inserting `setup<Feature>Dependencies(GetIt.instance)` and the matching import above the `// mason:core-feature-regis` anchor each time `mason make feature` is run.
...
**Generated l10n base (`lib/l10n/`):**

| File | Purpose |
|---|---|
| `app_en.arb` | Merged base ARB. Populated automatically by the feature hook and by `scripts/merge_l10n.dart` |
| `l10n.yaml` | `flutter gen-l10n` config — outputs to `lib/l10n/` by default due to `synthetic-package: false` |

**Generated helper script (`scripts/`):**

| File | Purpose |
|---|---|
| `scripts/merge_l10n.dart` | Scans all `lib/features/*/l10n/*_en.arb` files and merges their keys into `lib/l10n/app_en.arb`. Run whenever you add strings manually. |
...
> **Auto-wiring:** if `lib/main.dart` was generated with `--setup_main true`, the hook also
> inserts `setupUserProfileDependencies(GetIt.instance)` and its import automatically.
...
| `setupStatus` | `AppSetupStatus` | ❌ | `unknown` / `initializing` / `initialized` |
...
This wizard uses a **merged-ARB** approach: all translatable strings ultimately
live in `lib/l10n/app_en.arb` (and `app_xx.arb` for other locales), which are the files `flutter gen-l10n` reads.

### Pubspec Configuration
Ensure your `pubspec.yaml` has localization enabled:
```yaml
dependencies:
  flutter:
    sdk: flutter
...
```
lib/features/<feature>/l10n/<feature>_en.arb   ← you write strings here
         │
         │  (auto-merged by)  
         ▼
 lib/l10n/app_en.arb                           ← merged base file
         │
         │  flutter gen-l10n
         ▼
 lib/l10n/app_localizations.dart               ← generated Dart code directly into lib/l10n
```

### When are keys merged?

| Trigger | Keys merged |
|---|---|
| `mason make feature --feature_name x` | Initial keys from `x_en.arb` merged automatically by the post-gen hook |
| Manually after adding strings | Run **`dart run scripts/merge_l10n.dart`** |

### Day-to-day l10n loop

```bash
# 1. Add / edit strings in your feature ARB
vim lib/features/call/l10n/call_en.arb

# 2. Merge all feature ARBs → app_en.arb
dart run scripts/merge_l10n.dart

# 3. Generate Dart localization code
flutter gen-l10n
```

> `merge_l10n.dart` is **idempotent**: running it multiple times will update
> existing keys to their latest value without creating duplicates.

### Adding a new locale

1. Create `lib/features/<feature>/l10n/<feature>_fr.arb` (matching locale code)
2. Update `lib/l10n/l10n.yaml` → add the locale to `preferred-supported-locales` if needed
3. Run `dart run scripts/merge_l10n.dart` (update the script's `_en` glob if you add `_fr` support)
...
`lib/main.dart` (generated with `--setup_main true`) contains:

```dart
  registerCoreDependencies(GetIt.instance);
  // mason:core-feature-regis    ← patched by: feature brick post_gen hook
...
```bash
# 1. Scaffold the core architecture (--setup_main generates main.dart + app.dart)
mason make core \
  --use_cases true \
  --network true \
  --base_url "https://api.example.com" \
...
# 2. Scaffold a feature (auto-merges l10n keys + patches main.dart DI)
mason make feature --feature_name user_profile
...
# 11. Merge l10n + generate localizations
dart run scripts/merge_l10n.dart
flutter gen-l10n
